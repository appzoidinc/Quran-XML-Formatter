...
namespace XMLFormatter
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            QuranCore.Properties p = await GetTextWithoutDiacritics("quran-simple-clean.txt");

            QuranCore.Quran q = QuranCore.Convert.FromCustomXML("custom-quran.xml", QuranType.Normal);
            QuranCore.Convert.AddProperties(p, ref q);
            QuranCore.Convert.WriteQuranXML(q, @"custom2.xml");
        ...
        static async Task<Properties> GetTextWithoutDiacritics(string path)
        {
            string[] lines = await System.IO.File.ReadAllLinesAsync(path);
